...
public class LobbiesController : MonoBehaviour
{
    ...
    private void SessionsUpdatesHandler(EventSource eSource, Message message)
    {
        ...
        switch (sessionType)
        {
            ...
            case "ended":
                if (dataJsonObj[sessionId].ToString() == "null") { RemoveSession(sessionType, sessionId);/* Messenger.Broadcast(SessionshipNotifications.OnRemovedFromWishers, sessionId);*/ }
                else if (dataJsonObj[sessionId].AsBool == true) AddSession(sessionType, sessionId); 
                break;
    ...
    private void AddSessionListener(SessionData session)
    {
